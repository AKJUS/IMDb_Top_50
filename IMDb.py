...
# IMDb is protected by AWS WAF Bot Control, which blocks Selenium/Playwright
# outright (their CDP automation fingerprint gets flagged before any JS
# challenge is even offered - see project history). obscura
# (https://github.com/h4ckf0r0day/obscura) is a from-scratch Rust browser
# engine that isn't Chromium/CDP-based, so it doesn't trip that fingerprint;
# its --stealth mode genuinely executes the WAF's JS challenge via V8 and
# obtains a valid token cookie, which then persists across requests in the
# same --storage-dir for the rest of the run.
OBSCURA_BIN = os.environ.get("OBSCURA_BIN", "obscura")
_OBSCURA_STORAGE_DIR = None


def _obscura_storage_dir() -> str:
    global _OBSCURA_STORAGE_DIR
    if _OBSCURA_STORAGE_DIR is None:
        _OBSCURA_STORAGE_DIR = tempfile.mkdtemp(prefix="obscura-storage-")
    return _OBSCURA_STORAGE_DIR


def cleanup_obscura_storage() -> None:
    if _OBSCURA_STORAGE_DIR:
        shutil.rmtree(_OBSCURA_STORAGE_DIR, ignore_errors=True)


def warm_up_obscura() -> None:
    """
    Solve IMDb's AWS WAF Bot Control challenge once at startup so every
    later fetch in this run can reuse the resulting token cookie directly.
    """
    print("  Warming up obscura (solving IMDb's bot-check challenge)...")
    fetch_html(IMDB_BASE_URL, wait=15)


def fetch_html(url: str, wait: int = 15, timeout: int = 60) -> str | None:
    """Fetch a URL's fully rendered HTML via the obscura headless browser."""
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--stealth", "--dump", "html",
                "--wait", str(wait), "--timeout", str(timeout),
                "--storage-dir", _obscura_storage_dir(),
                "-o", out_path, "-q",
            ],
            capture_output=True, text=True, timeout=timeout + 30,
        )
        if result.returncode != 0:
            print(f"    obscura fetch failed for {url}: {result.stderr.strip()[:300]}")
            return None
        with open(out_path, "r", errors="replace") as f:
            return f.read()
    except subprocess.TimeoutExpired:
        print(f"    obscura fetch timed out for {url}")
        return None
    finally:
        try:
            os.remove(out_path)
...
def _fetch_soup(url: str, finder, retries: int = 2):
    """
    Fetch url and return its parsed soup once finder(soup) finds the expected
    data script tag, retrying if IMDb serves a bot-check interstitial instead
    of the real page. Returns None if it never shows up.
    """
    soup = None
    for attempt in range(1, retries + 1):
        ...
        if html:
            ...
            if finder(soup):
                return soup
        print(f"    Attempt {attempt}/{retries}: {url} not ready, retrying...")
...
def enrich_title_data(url: str) -> dict:
    """
...
def enrich_items(items: list[dict], link_key: str = "link", existing: list[dict] = None) -> list[dict]:
    ...
    if existing:
        for ex in existing:
            ...
            if not url:
                continue
            fields = {k: v for k, v in ex.items() if k in _ENRICH_FIELDS}
            # Guard against a schema change leaving only a stray
            # "last_updated" behind (e.g. from a differently-shaped record
            # for the same title) - that alone must never look like a
            # real cache hit.
            if len(fields) > 1:
                cache[_title_id_from_url(url)] = fields
    ...
    for i, item in enumerate(to_fetch):
        ...
        enrichment = enrich_title_data(url)
